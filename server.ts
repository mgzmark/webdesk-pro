    ...
    ws.on("message", (message) => {
      try {
        ...
        switch (data.type) {
          ...
          case "mouse_event":
          case "keyboard_event":
            if (data.type === 'mouse_event' && (data.mouseType === 'mousedown' || data.mouseType === 'mouseup' || data.mouseType === 'mouseup_global')) {
              console.log(`[Server] Forwarding ${data.mouseType} from ${id} to ${data.targetId}`);
            }
            // 将事件转发给绑定的本地 Node.js 脚本 (如果存在)
